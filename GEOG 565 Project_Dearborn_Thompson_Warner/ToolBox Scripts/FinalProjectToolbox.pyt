...
class Clip_Buffer(object):
    ...
    def getParameterInfo(self):
        ...
        soils = arcpy.Parameter(
            ...
            parameterType="Optional",
            direction="Input")
        zoning = arcpy.Parameter(
            displayName="Zoning Input",
            name="zoning",
            datatype="DEShapefile",
            parameterType="Optional",
            direction="Input")
        ...
        return [ workspace, out_path, clip_feature, input_streams, dissolve_field_streams, input_bodies,
            dissolve_field_bodies, input_landcover, lc_shapefile, wria_basin, soils, zoning ]
    ...
    def execute(self, parameters, messages):
        ...
        zoning = parameters[i].valueAsText
        try:
            ...
            if wria_basin != "":
                ...
                pass
            if zoning != "":
                clip_zoning = os.path.join(out_path, os.path.basename("clip_zoning"))
                arcpy.Clip_analysis(zoning, clip_feature, clip_zoning)
                clip_dict.append(clip_zoning)
                arcpy.AddMessage(clip_zoning + " clipped!")
            elif zoning == "":
                pass
            ...
            arcpy.Delete_management(clip_zoning)
